...
export type TableProperty<T extends Table> =
	T extends DataTable ? DataTableProperty<T>:
		T extends RelationalTable ? RelationalTableProperty<T> :
			TicketAssignee;

export type OurQueryResultRow =
	UserAccount | Project | Ticket | Metric | MetricOption | UserProject | TicketAssignee
	| { [column: string]: string } | { [column: string]: number };
